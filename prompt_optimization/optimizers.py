...
class ProTeGi(PromptOptimizer):
    ...
    def _sample_error_str(self, texts, labels, preds, task, n=4):
        ...
        for i, (t, l, p) in enumerate(zip(sample_texts, sample_labels, sample_preds)):
            error_string += f'## 誤った例{error_idx+1}\n'
            error_string += f'指示プロンプト: \"{t.strip()}\"\n\n正解: {task.stringify_prediction(l)}\n誤った回答: {task.stringify_prediction(p)}\n\n'
            error_idx += 1
    ...
    def _get_gradients(self, prompt, error_string, num_feedbacks=5, n=1):
        ...
        gradient_prompt = f"""
        今私はゼロショット分類器（a zero-shot classfier）のプロンプトを書こうとしています。

        現在のプロンプト：
        "{prompt}"

        しかし、このプロンプトは以下に示す質問の場合に誤った回答をします：
        {error_string}

        なぜこのプロンプトがこれらの例を間違えたか、私は{num_feedbacks}個の理由を考えました。
        以下に列挙するそれぞれの理由は、<START>と<END>で囲われています。
        """
        gradient_prompt = '\n'.join([line.lstrip() for line in gradient_prompt.split('\n')])
        res = utils.chatgpt(gradient_prompt, n=n, temperature=self.opt['temperature'])
        feedbacks = []
        new_prompts = []
        for r in res:
    ...
    def apply_gradient(self, prompt, error_str, feedback_str, steps_per_gradient, n=1):
        ...
        上記の質問を誤った例から考えられる現状のプロンプトの問題を以下に示します。
        {feedback_str}

        上記の情報から、私は{steps_per_gradient}個の異なる改善されたプロンプトを書きました。
        それぞれのプロンプトは、<START>と<END>で囲まれています。
        """
        transformation_prompt = '\n'.join([line.lstrip() for line in transformation_prompt.split('\n')])
        res = utils.chatgpt(transformation_prompt, n=n, temperature=self.opt['temperature'])
        new_prompts = []
        for r in res:
    ...
    def generate_synonyms(self, prompt_section, n=3):
        ...
        rewriter_prompt = f"以下の入力で示す指示プロンプトの意味を保ちながら、異なる指示プロンプトのバリエーションを出力してください。\n\n入力: {prompt_section}\n\n出力:"
        new_instructions = utils.chatgpt(rewriter_prompt, n=n, temperature=self.opt['temperature'])
